#!/usr/bin/env python3
"""Rebuild the 9:16 ad with a natural male English voice."""
...
BUILD = ROOT / "build"
OUT = ROOT / "taxi-and-fly-google-ads-9x16-en.mp4"
VO_DIR = BUILD / "vo_en"
ARTIFACT = Path("/opt/cursor/artifacts/taxi_and_fly_english.mp4")

sys.path.insert(0, str(ROOT))
from build_video import FONT, FONT_REG, duration, ff, kenburns, make_audio, mix, xfade_concat  # noqa: E402

LINES = [
    "Wherever you are, and you're coming to Athens.",
    "You fill in the form.",
    "You enter the destination you want.",
    "You book a ride with a driver.",
    "When you arrive at the airport,",
    "you tap the button for the driver to pick you up.",
    "You tell the driver which arrivals exit number you're at.",
    "He comes and picks you up.",
    "He takes you safely to your destination.",
    "You can write how your ride and the driver were, so we keep improving.",
    "END",
]

AIRPORT_SENTENCE = (
    "When you arrive at the airport, you tap the button for the driver to pick you up."
)
...
def split_on_pause(src: Path, left: Path, right: Path) -> None:
    ...
    cut = next((s for s in starts if 1.35 < s < d * 0.62), None)
    if cut is None:
...
def make_end_card_png(dst: Path) -> None:
    ...
    center("To and from the airport", 860, f_hero, (255, 255, 255))
...
async def main() -> int:
    ...
    for i, line in enumerate(LINES):
        ...
        if i == 10:
            ...
            await speak("Taxi and Fly.", a, rate="-6%")
            await speak("To and from the airport.", b, rate="-14%")
            await speak("With professional taxi drivers.", c, rate="-8%")
